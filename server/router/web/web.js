module.exports=app=>{
    ...
    // const Article=mongoose.model('Article',Sch)
    // const Category=mongoose.model('Category')
    const Article=require('../../plugin/models/Article')
    const Category=require('../../plugin/models/Category')
    const Hero=require('../../plugin/models/Hero')
    const Ad=require('../../plugin/models/Ad')
    router.get('/news/init',async (req,res)=>{
        ...
        const newslists=["体验服爆料丨父子相聚，蒙恬将军带着玄雍军阵登场", "探秘瑶·新皮肤诞生地，敦煌动画正在上映", "星元部件爆料丨换上新装闪耀舞台，偶像歌手化身魔法少女", "王者荣耀×敦煌︱又有新皮肤啦！？", "《王者荣耀》皮肤团2020年春季招新公告", "1元开启暖春七日登录礼活动异常说明公告", "4月17日体验服停机修复公告", "4月17日体验服停机更新公告", "体验服奖励开放公告", "4月15日外挂专项打击公告", "充值1元即可开启暖春七日登录好礼", "天魔缭乱限时返场 多重好礼等你来领", "橘右京全新史诗皮肤上架 SNK皮肤齐聚峡谷福利大放送", "【预告】荣耀中国节·轻风戏纸鸢，参与必得纸鸢回城特效（永久）", "参与“S19战令集结号”，赢80级直升经验大礼", "关于责成TS俱乐部就欠薪问题进行说明的通知", "《一招即浪》第二期：逆天牛魔王最浪打法，城市赛教你一招制敌", "为战队争夺荣誉！城市联赛战队赛通道报名开启！", "高校联赛分站赛即将打响 今年又有多少好玩的活动值得期待？", "《一招即浪》第一期：白起强势归来，教你如何化身“边路杀神”"]


        const newslist=newslists.map(title=>{
            const  randlist=cats.slice(0).sort((a,b)=>Math.random()-0.5)
             return {
                 categroies:randlist.slice(0,2),
                 title:title
             }
         })
        ...
        await  Hero.deleteMany({})
         const rawdata=[{"name":"热门",
             ...
             for (let cat of rawdata)
             {
               ...
               cat.heros=cat.heros.map(el=>{
                     el.categroies=[category]
                     return el
                 })
    ...
    router.get('/news/list',async (req,res)=>{
        ...
        const suncats=cats.map(v=>v._id)
        cats.unshift({
            name:'热门',
            newslist: await Article.find().where({
                categroies:{$in:suncats}
            }).populate('categroies').limit(5).lean()

        })
              console.log(cats)
        cats.map(cat=>{
            cat.newslist.map(news=>{
                news.catename=(cat.name==='热门')?news.categroies[0].name :cat.name
                return news
            })
            return cat
        })
    ...
    router.get('/heroes/list',async (req,res)=>{
        ...
        cats.unshift({
            ...
            heroeslist: await Hero.find().where({
                categroies:{$in:suncats}
            }).populate('categroies').limit(10).lean()
    ...
    router.get('/articles/:id',async (req,res)=>{
        ...
        data.related=await Article.find().where({
            categroies: {$in:data.categroies}
        }).limit(2)
        res.send(data)
    })
    router.get('/heroses/:id',async (req,res)=>{
        const data=await Hero.findById(req.params.id).populate('categroies items1 items2 partens.hero').lean()
        console.log(data)
        res.send(data)
    })
    router.get('/ads/list',async (req,res)=>{
        const data=await Ad.find().lean()
        res.send(data)
    })
